...
- Calculating Area: Temporary Variables in a Session
```bash
length=20
breadth=10
...
- Permanent Variables (for demonstration, but not typical):
  - Add the assignments to `~/.bashrc`, then `source` the file. This is uncommon for transient data like this; variables are usually set in scripts.
